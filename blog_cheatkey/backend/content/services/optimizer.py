...
class ContentOptimizer:
    ...
    def _create_ultra_seo_prompt(self, content, keyword, custom_morphemes, analysis_result):
        ...
        1️⃣ 글자수 목표: 
        • 최종 글자수(공백 제외)를 {target_min_chars}-{target_max_chars}자 사이로 맞춰주세요.
        • 현재 글자수: {analysis_result['char_count']}자

        2️⃣ 목표 형태소 출현 빈도 가이드라인:
        • 주요 키워드 '{keyword}'와 관련된 핵심 형태소({morpheme_instruction_text})가 지정된 횟수만큼 자연스럽게 포함되도록 노력해주세요.
        • 현재 목표 형태소 분석 결과:
